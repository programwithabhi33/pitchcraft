  ...
  return (
    <motion.aside
      ...
      animate={{ 
        ...
        // On very small screens, we might want to hide it or use a different width
        // but for now, let's keep it consistent
      }}
      transition={{ type: 'spring', stiffness: 300, damping: 30 }}
      className="relative flex-shrink-0 h-screen sticky top-0 flex flex-col bg-[#18181B] border-r border-[#27272A] z-30"
    >
      {/* Internal wrapper for content to handle overflow during width animation */}
      ...
      {/* ── Collapse toggle (Placed OUTSIDE overflow-hidden wrapper) ──────────────────────── */}
      <button
        id="sidebar-collapse-btn"
        onClick={toggleSidebar}
        title={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}
        className="absolute -right-3 top-20 w-6 h-6 rounded-full bg-[#27272A] border border-[#3F3F46] flex items-center justify-center text-[#71717A] hover:text-[#F4F4F5] hover:bg-[#3F3F46] transition-all shadow-lg z-50 cursor-pointer"
      >
