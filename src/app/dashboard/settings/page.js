        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-[#71717A] leading-relaxed">
              ...
              . You will lose access to all your saved emails and outreach
              history forever.
            </p>
          </div>
          ...
          <div className="flex flex-col w-full gap-3">
            <button
              onClick={onConfirm}
              disabled={isDeleting}
              className="w-full py-3.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-rose-600/20 active:scale-[0.98] cursor-pointer"
            >
              {isDeleting ? (
            ...
            <button
              onClick={onClose}
              disabled={isDeleting}
              className="w-full py-3.5 bg-[#27272A] hover:bg-[#3F3F46] text-[#F4F4F5] font-semibold rounded-xl transition-all cursor-pointer"
            >
  ...
  useEffect(() => {
    if (user) {
      const normalizedTone = user.senderTone
        ? user.senderTone.charAt(0).toUpperCase() +
          user.senderTone.slice(1).toLowerCase()
        : "Friendly";
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <DashboardHeader
        ...
        subtitle="Manage your identity and outreach defaults"
      />
        ...
        <div className="max-w-4xl mx-auto">
              ...
              <div className="p-8 bg-[#1E1033]/20 border-b md:border-b-0 md:border-r border-[#27272A] flex flex-col items-center text-center space-y-4">
                ...
                <div>
                  <h3 className="text-base font-bold text-[#F4F4F5]">
                    {isFetching ? "Loading..." : user?.name || "User"}
                  </h3>
                ...
                <form onSubmit={handleSave} className="space-y-6">
                  <div className="grid grid-cols-1 gap-5">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold uppercase tracking-wider text-[#A1A1AA] flex items-center gap-2">
                        <User size={13} className="text-[#7C3AED]" /> Your Full
                        Name
                      </label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold uppercase tracking-wider text-[#A1A1AA] flex items-center gap-2">
                        <Briefcase size={13} className="text-[#0EA5E9]" /> Your
                        Service / Title
                      </label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1.5 pt-2">
                      ...
                      <div className="flex items-center gap-2 bg-[#09090B] p-1 rounded-xl border border-[#27272A] w-fit relative z-0">
                        {TONE_OPTIONS.map((tone) => {
                          const isSelected =
                            formData.senderTone.toLowerCase() ===
                            tone.toLowerCase();
                          return (
                            <button
                              key={tone}
                              type="button"
                              onClick={() =>
                                setFormData({ ...formData, senderTone: tone })
                              }
                              className={`relative px-4 py-1.5 text-xs font-semibold rounded-lg transition-colors duration-200 z-10 cursor-pointer ${
                                isSelected
                                  ? "text-[#F4F4F5]"
                  ...
                  <div className="pt-4 border-t border-[#27272A] flex items-center justify-between">
                    ...
                    <button
                      type="submit"
                      disabled={isSaving}
                      className="btn-violet px-8 py-3 text-sm font-bold flex items-center gap-2 min-w-[160px] justify-center cursor-pointer"
                    >
                      {isSaving ? (
          ...
          <div className="mt-8 flex justify-center">
            <button
              ...
              className="group text-xs font-medium text-[#52525B] hover:text-rose-500 transition-colors flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-rose-500/5 cursor-pointer"
            >
